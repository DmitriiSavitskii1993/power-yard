...
const CALC_DATA = {
  ...
  /* --- Китай: дефолтные плечи логистики (в валюте цены), правятся на экране --- */
  china: {
    defaultLeg1Usd: 1000,   // Китай → Бишкек
    defaultLeg2Usd: 1800,   // Бишкек → СПб
  },

  /* --- Грузия: дефолтная логистика до РФ (в $), правится на экране --- */
  georgia: {
    defaultLogisticsUsd: 1000,
  },

  /* --- Комиссия платёжного агента: % от инвойса за авто (галочка на форме) --- */
  paymentAgent: { percent: 0.02 },
  ...
  customsModeOptions: {
    ...
    ge: [ {id:'phys_rf',label:'Физлицо РФ'}, {id:'jur',label:'Юрлицо (НДС 22%)'}, {id:'phys_kg',label:'Через КГ'}, {id:'manual_belka',label:'Через Белку'} ],
  },
